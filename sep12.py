        ...
        try:
            ...
            def build_da_only_model(prices, demand_profile, soc0, η_self, boiler_eff, 
                ...
                for t in range(T):
                    ...
                    # Determine previous SOC for charging limit check
                    if t == 0:
                        prev_soc = soc0
                    else:
                        prev_soc = soc[t-1]
                    ...
                    model += prev_soc <= effective_smax_da + M * (1 - can_charge[t])
                    model += prev_soc >= effective_smax_da - M * can_charge[t] + 0.001
                    ...
                    if enable_curve and curve_params is not None:
                        if m_charge is not None:
                            ...
                            model += p_el_da[t] <= m_charge * prev_soc + b_charge
                        if m_discharge is not None:
                            model += p_th[t] <= Pmax_th
                            model += p_th[t] <= m_discharge * prev_soc + b_discharge
                    ...
                    if status == 1:
                        ...
                        for t in range(T):
                            ...
                            trade_record = {
                                ...
                                'soc': sim_results['final_soc_trajectory'][t],
                        ...
                        results.append({
                            "day": day, "cost": reported_cash_flow_cost, "savings": savings, "soc_end": sim_results['final_soc_trajectory'][-1], 
                        ...
                        soc0 = sim_results['final_soc_trajectory'][-1]
                        ...
                        results.append({
                            ...
                            "afrr_cap_revenue": 0, "afrr_energy_revenue": 0
                        })
