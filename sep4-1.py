        ...
        try:
            ...
            def simulate_afrr_energy_on_da_plan(da_plan, daily_clearing_prices, daily_activation_profile,
                ...
                for t in range(T):
                    ...
                    if p_el_afrr > 0.01:
                        ...
                        net_cost_per_mwh = C_grid - clearing_price  # If clearing_price > C_grid, this is negative (we profit)
            ...
            if st.button("🚀 Run Optimization", type="primary"):
                ...
                for idx, day_plan in enumerate(da_plan):
                    ...
                    # ==== FINAL AGGREGATION ====
                    # Calculate costs and revenues from all three markets
                    ...
                    # aFRR energy: calculate actual electricity cost and thermal energy gained
                    afrr_energy_elec_cost = 0
                    afrr_thermal_energy_gained = 0
                    afrr_energy_revenue = 0
                    
                    for detail in afrr_energy_details:
                        power = detail['power']
                        clearing_price = detail['clearing_price']
                        thermal_gained = detail['thermal_gained']
                        
                        # Actual electricity cost for aFRR charging (grid charges + clearing price)
                        elec_cost_for_interval = (C_grid + clearing_price) * power * Δt
                        afrr_energy_elec_cost += elec_cost_for_interval
                        afrr_thermal_energy_gained += thermal_gained
                        
                        # aFRR energy revenue = savings vs gas for this thermal energy
                        avoided_gas_cost = thermal_gained * (C_gas / boiler_efficiency)
                        afrr_energy_revenue += (avoided_gas_cost - elec_cost_for_interval)
                    
                    # Adjust gas cost: subtract the thermal energy provided by aFRR charging
                    adjusted_gas_cost = gas_cost - (afrr_thermal_energy_gained * (C_gas / boiler_efficiency))
                    
                    # Total system cost = DA electricity + adjusted gas + aFRR electricity - capacity revenue
                    total_system_cost = elec_cost_da + adjusted_gas_cost + afrr_energy_elec_cost - day_plan['afrr_cap_revenue']
                    ...
                    for t in range(len(day_plan['prices'])):
                        ...
                        for detail in afrr_energy_details:
                            if detail['interval'] == t:
                                ...
                                # Calculate aFRR cost for this interval
                                afrr_net_cost_interval = (C_grid + detail['clearing_price']) * detail['power'] * Δt
                                break
