        ...
        try:
            ...
            def build_thermal_model(prices, demand_profile, soc0, η_self, boiler_eff, 
                ...
                afrr_energy_costs = lpSum([
                    (C_grid - (safe_float_convert(afrr_clearing_prices[t]) if afrr_clearing_prices is not None and t < len(afrr_clearing_prices) else 0.0)) 
                    * p_el_afrr[t] * Δt 
                    for t in range(T)
                ])
            ...
            if st.button("🚀 Run Optimization", type="primary"):
                ...
                for idx, (_, row) in enumerate(df_processed.iterrows()):
                    ...
                    if len(prices) != len(demand_profile):
                        st.warning(f"Data length mismatch for {day}: {len(prices)} prices vs {len(demand_profile)} demand values. Padding with zeros...")
                        if len(demand_profile) < len(prices):
                            # Pad demand with zeros for missing intervals
                            demand_profile = np.concatenate([demand_profile, np.zeros(len(prices) - len(demand_profile))])
                        else:
                            # Truncate demand to match prices
                    ...
                    if status == 1:
                        ...
                        for t in range(len(prices)):
                            if p_el_afrr[t].value() > 0.01:  # Only count when actually charging
                                ...
                                net_cost_for_interval = net_cost_per_mwh * power * Δt
                        ...
                        afrr_energy_cost = sum([d['net_cost'] for d in afrr_energy_details]) if afrr_energy_details else 0
                        ...
                        for t in range(len(prices)):
                            ...
                            if p_el_afrr[t].value() > 0.01:
                                clearing_price = safe_float_convert(daily_clearing_prices[t]) if daily_clearing_prices is not None else 0.0
                                afrr_net_cost_interval = (C_grid - clearing_price) * p_el_afrr[t].value() * Δt
                        ...
                        results.append({
                            "day": day, 
                            "cost": fallback_gas_cost,  # Cost equals gas baseline (all demand met by gas)
                            "savings": 0,  # No savings since using only gas
                            "soc_end": soc0, 
                            "elec_energy": 0, 
                            "gas_energy": sum(demand_profile) * Δt,  # All thermal demand met by gas
                            "is_holiday": is_holiday, 
                            "gas_baseline_daily": gas_baseline_daily, 
                            "elec_cost_da": 0, 
                            "gas_cost": fallback_gas_cost,
                            "afrr_cap_revenue": 0, 
                            "afrr_energy_revenue": 0
                        })
                        # Also add empty trade records for this day
                        for t in range(len(prices)):
                            interval_hour, interval_min = divmod(t * 15, 60)
                            time_str = f"{interval_hour:02d}:{interval_min:02d}:00"
                            trade_record = {
                                'date': day, 'time': time_str, 'interval': t, 'da_price': prices[t],
                                'total_elec_cost': prices[t] + C_grid, 'p_el_heater': 0,
                                'p_el_da': 0, 'p_el_afrr': 0,
                                'p_th_discharge': 0, 'p_gas_backup': 0,
                                'soc': soc0, 'elec_cost_interval': 0,
                                'gas_cost_interval': 0,
                                'total_cost_interval': 0,
                                'is_hochlast': False, 'is_holiday': is_holiday,
                                'is_charging': False, 'is_discharging': False,
                                'using_gas': False, 'demand_th': demand_profile[t],
                                'is_in_afrr_market': False,
                                'afrr_energy_won': 0
                        }
                        all_trades.append(trade_record)
